...
function streamParser(filename: string): any[] {
    const fileStream = fs.createReadStream(filename, { encoding: "utf8", highWaterMark: 64 * 1024 }); // Read in 64KB chunks
    const rl = readline.createInterface({
        input: fileStream,
        crlfDelay: Infinity
    });

    let lexerState: { hm: TokenInfo, prevType: string } = {
        hm: {
            "object": 0,
            "array": 0,
            "quote": 0,
    ...
    let parserState = initParseState();

    rl.on('line', (line: string) => {
        // Lexing the line
        const { tokens, hm, prevType } = jsonLexerLine(line, lexerState.prevType, lexerState.hm);
        lexerState.hm = hm;
        lexerState.prevType = prevType;

        // Parsing the tokens
        for (const token of tokens) {
            parserState = processSingleToken(token, parserState);
        }
    });

    rl.on('close', () => {
        console.log("Finished reading the file.");
        console.log("Tokens state:", lexerState);
        console.log("Parse results:", parserState.results);
    });

    return parserState.results;
}

streamParser("./output_postcode_partial.txt");
